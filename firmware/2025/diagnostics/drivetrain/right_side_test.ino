/*
  Right Side Drive Test (D9/D10)
  Drives the right side motor group only and reports encoder counts.
*/

#include <Arduino.h>

const uint8_t IN1_PIN = 9;
const uint8_t IN2_PIN = 10;
...
void setup() {
  ...
  setupEncoders();
  Serial.println(F("Right side test"));
}
...
void loop() {
  ...
  delay(RUN_MS);
  printEncoderSnapshot("Right FWD", captureEncoderSnapshot());
  ...
  delay(RUN_MS);
  printEncoderSnapshot("Right REV", captureEncoderSnapshot());
